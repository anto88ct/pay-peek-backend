...
public class BiometricController {
    ...
    /**
     * GET /api/auth/biometric/register-challenge/{email}
     */
    @GetMapping("/register-challenge/{email}")
    public ResponseEntity<?> getRegisterChallenge(
            @PathVariable String email,
            HttpServletRequest request) {
        try {
            log.debug("Generating register challenge for email: {}", email);
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(buildErrorResponse("USER_NOT_FOUND", "Utente non trovato", HttpStatus.NOT_FOUND.value(), request));
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(buildErrorResponse("BIOMETRIC_NOT_AVAILABLE", e.getMessage(), HttpStatus.BAD_REQUEST.value(), request));

        } catch (Exception e) {
            log.error("Error generating register challenge", e);
            auditLogger.log(email, "BIOMETRIC_REGISTER_CHALLENGE_ERROR", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(buildErrorResponse("SERVER_ERROR", "Errore nella generazione della challenge", HttpStatus.INTERNAL_SERVER_ERROR.value(), request));
        }
    }
    ...
    public ResponseEntity<?> registerBiometric(
        ...
        try {
            log.debug("Registering biometric credential for email: {}", request.getEmail());
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(buildErrorResponse("USER_NOT_FOUND", "Utente non trovato", HttpStatus.NOT_FOUND.value(), servletRequest));

        } catch (BiometricAuthException e) {
            auditLogger.log(request.getEmail(), "BIOMETRIC_REGISTER_FAILED", "invalid_credential");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(buildErrorResponse("INVALID_CREDENTIAL", e.getMessage(), HttpStatus.BAD_REQUEST.value(), servletRequest));

        } catch (Exception e) {
            log.error("Error registering biometric", e);
            auditLogger.log(request.getEmail(), "BIOMETRIC_REGISTER_ERROR", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(buildErrorResponse("SERVER_ERROR", "Errore nella registrazione biometrica", HttpStatus.INTERNAL_SERVER_ERROR.value(), servletRequest));
        }
    }

    /**
     * GET /api/auth/biometric/login-challenge/{email}
     */
    @GetMapping("/login-challenge/{email}")
    public ResponseEntity<?> getLoginChallenge(
            @PathVariable String email,
            HttpServletRequest request) {
        try {
            log.debug("Generating login challenge for email: {}", email);
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(buildErrorResponse("BIOMETRIC_NOT_REGISTERED", e.getMessage(), HttpStatus.BAD_REQUEST.value(), request));

        } catch (Exception e) {
            log.error("Error generating login challenge", e);
    ...
    public ResponseEntity<?> loginBiometric(
            ...
            HttpServletRequest servletRequest) {
        try {
            log.debug("Processing biometric login for email: {}", request.getEmail());
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(buildErrorResponse("BIOMETRIC_AUTH_FAILED", e.getMessage(), HttpStatus.UNAUTHORIZED.value(), servletRequest));

        } catch (Exception e) {
            log.error("Error processing biometric login", e);
            auditLogger.log(request.getEmail(), "BIOMETRIC_LOGIN_ERROR", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(buildErrorResponse("SERVER_ERROR", "Errore nell'autenticazione biometrica", HttpStatus.INTERNAL_SERVER_ERROR.value(), servletRequest));
        }
    }

    /**
     * GET /api/auth/biometric/check/{email}
     */
    @GetMapping("/check/{email}")
    public ResponseEntity<?> checkBiometricRegistration(
            @PathVariable String email,
            HttpServletRequest request) {
        try {
            log.debug("Checking biometric registration for email: {}", email);
            ...
            log.error("Error checking biometric registration", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(buildErrorResponse("SERVER_ERROR", "Errore nella verifica della biometria", HttpStatus.INTERNAL_SERVER_ERROR.value(), request));
        }
    }

    /**
     * ✅ METODO HELPER per ErrorResponseDto
     */
    private ErrorResponseDto buildErrorResponse(String type, String message, int statusCode, HttpServletRequest request) {
